...
class WordIndexer(QWidget):
    ...
    def initUI(self):
        self.setWindowTitle("단어 색인 프로그램")  # 창 제목 설정
        ...
        # 색인 생성 버튼
        self.indexButton = QPushButton("색인 생성", self)
        self.indexButton.clicked.connect(self.create_index)  # 버튼 클릭 시 색인 생성 함수 호출
    ...
    def create_index(self):
        ...
        if target_word:
            ...
            if self.positions:
                ...
                result_text = f"'{target_word}'의 위치: {self.positions}"
            else:
